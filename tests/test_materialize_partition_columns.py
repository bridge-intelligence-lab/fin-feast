...
def test_ensure_partition_columns(tmp_path: Path) -> None:
    ...
    for freq in ("minute", "daily"):
        d = project_root / "data" / "offline" / "current" / freq / "symbol=X:BTCUSD" / "date=2025-01-01"
    ...
    for freq in ("minute", "daily"):
        p = project_root / "data" / "offline" / "current" / freq / "symbol=X:BTCUSD" / "date=2025-01-01" / "data.parquet"
